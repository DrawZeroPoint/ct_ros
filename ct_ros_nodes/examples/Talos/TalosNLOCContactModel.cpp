...
const size_t njoints = ct::rbd::Talos::Kinematics::NJOINTS;

typedef ct::rbd::FloatingBaseFDSystem<ct::rbd::Talos::Dynamics, false> TalosSystem;
typedef ct::rbd::EEContactModel<typename TalosSystem::Kinematics> ContactModel;
std::shared_ptr<ContactModel> contactModel;
typedef ct::models::Talos::TalosWithContactModelLinearizedForward CodegenLinModel;
typedef ct::core::LinearSystem<TalosSystem::STATE_DIM, TalosSystem::CONTROL_DIM> LinearSystem;
...
{
    ros::init(argc, argv, "talos_nloc_with_contacts");
    ros::NodeHandle nh("~");

    ct::ros::RBDStatePublisher statePublisher(ct::models::Talos::urdfJointNames(), "/base_link", "/world");
    statePublisher.advertise(nh, "/joint_states", 10);

    typedef ct::rbd::Talos::Dynamics TalosDynamics;
    ...
    std::shared_ptr<TalosSystem> system(new TalosSystem);
    ...
    if (useCodegenModel)
        ...
        linSystem = std::shared_ptr<LinearSystem>(
            new ct::core::SystemLinearizer<TalosSystem::STATE_DIM, TalosSystem::CONTROL_DIM>(system));

    ROS_INFO("Initializing NLOC");
    FloatingBaseNLOCContactModel<TalosDynamics> nloc(costFunctionFile, configFile, system, linSystem);

    ct::core::Time timeHorizon;
    ct::core::loadScalar(configFile, "timeHorizon", timeHorizon);
    FloatingBaseNLOCContactModel<TalosDynamics>::FeedbackArray::value_type fb;
    ...
    FloatingBaseNLOCContactModel<TalosDynamics>::FeedbackArray fbTrajectory(N, -fb);
    FloatingBaseNLOCContactModel<TalosDynamics>::ControlVectorArray ffTrajectory(
        N, FloatingBaseNLOCContactModel<TalosDynamics>::ControlVector::Zero());
    FloatingBaseNLOCContactModel<TalosDynamics>::StateVectorArray x_ref(N + 1, x0.toStateVectorEulerXyz());
    ...
    ct::core::Integrator<TalosSystem::STATE_DIM> integratorForInit(system, ct::core::IntegrationType::RK4);
    FloatingBaseNLOCContactModel<TalosDynamics>::StateVectorArray x_integr;
    FloatingBaseNLOCContactModel<TalosDynamics>::ControlVectorArray u_integr;
    ...
    for (size_t i = 1; i < nIntSteps; i++)
    {
        ...
        std::shared_ptr<ct::core::ConstantStateFeedbackController<TalosSystem::STATE_DIM, TalosSystem::CONTROL_DIM>>
            controller(new ct::core::ConstantStateFeedbackController<TalosSystem::STATE_DIM, TalosSystem::CONTROL_DIM>(
    ...
    typename FloatingBaseNLOCContactModel<TalosDynamics>::StateVectorArray x_nloc;
