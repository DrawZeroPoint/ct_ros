...
const size_t njoints = ct::rbd::Walker::Kinematics::NJOINTS;

typedef ct::rbd::FloatingBaseFDSystem<ct::rbd::Walker::Dynamics, false> WalkerSystem;
typedef ct::rbd::EEContactModel<typename WalkerSystem::Kinematics> ContactModel;
std::shared_ptr<ContactModel> contactModel;
typedef ct::models::Walker::WalkerWithContactModelLinearizedForward CodegenLinModel;
typedef ct::core::LinearSystem<WalkerSystem::STATE_DIM, WalkerSystem::CONTROL_DIM> LinearSystem;
...
{
    ros::init(argc, argv, "walker_nloc_with_contacts");
    ros::NodeHandle nh("~");

    ct::ros::RBDStatePublisher statePublisher(ct::models::Walker::urdfJointNames(), "/torso", "/world");
    statePublisher.advertise(nh, "/joint_states", 10);

    typedef ct::rbd::Walker::Dynamics WalkerDynamics;
    ...
    std::shared_ptr<WalkerSystem> system(new WalkerSystem);
    ...
    if (useCodegenModel)
        ...
        linSystem = std::shared_ptr<LinearSystem>(
            new ct::core::SystemLinearizer<WalkerSystem::STATE_DIM, WalkerSystem::CONTROL_DIM>(system));

    ROS_INFO("Initializing NLOC");
    FloatingBaseNLOCContactModel<WalkerDynamics> nloc(costFunctionFile, configFile, system, linSystem);

    ct::core::Time timeHorizon;
    ct::core::loadScalar(configFile, "timeHorizon", timeHorizon);
    FloatingBaseNLOCContactModel<WalkerDynamics>::FeedbackArray::value_type fb;
    ...
    FloatingBaseNLOCContactModel<WalkerDynamics>::FeedbackArray fbTrajectory(N, -fb);
    FloatingBaseNLOCContactModel<WalkerDynamics>::ControlVectorArray ffTrajectory(
        N, FloatingBaseNLOCContactModel<WalkerDynamics>::ControlVector::Zero());
    FloatingBaseNLOCContactModel<WalkerDynamics>::StateVectorArray x_ref(N + 1, x0.toStateVectorEulerXyz());
    ...
    ct::core::Integrator<WalkerSystem::STATE_DIM> integratorForInit(system, ct::core::IntegrationType::RK4);
    FloatingBaseNLOCContactModel<WalkerDynamics>::StateVectorArray x_integr;
    FloatingBaseNLOCContactModel<WalkerDynamics>::ControlVectorArray u_integr;
    ...
    for (size_t i = 1; i < nIntSteps; i++)
    {
        ...
        std::shared_ptr<ct::core::ConstantStateFeedbackController<WalkerSystem::STATE_DIM, WalkerSystem::CONTROL_DIM>>
            controller(new ct::core::ConstantStateFeedbackController<WalkerSystem::STATE_DIM, WalkerSystem::CONTROL_DIM>(
    ...
    typename FloatingBaseNLOCContactModel<WalkerDynamics>::StateVectorArray x_nloc;
